...
struct table inspircd4_protocol_init_commands = {0};
struct table inspircd4_protocol_commands = {0};

char inspircd4_protocol_user_mode_types[UCHAR_MAX+1] = {
	['c'] = MODE_TYPE_NOARGS,
	['d'] = MODE_TYPE_NOARGS,
	['g'] = MODE_TYPE_NOARGS,
	['h'] = MODE_TYPE_NOARGS,
...
char inspircd4_protocol_channel_mode_types[UCHAR_MAX+1] = {
	['a'] = MODE_TYPE_USERS,
...
int init_inspircd4_protocol(void) {
	inspircd4_protocol_commands.array = malloc(0);

	set_table_index(&inspircd4_protocol_init_commands, STRING("CAPAB"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_init_handle_capab});
	set_table_index(&inspircd4_protocol_init_commands, STRING("SERVER"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_init_handle_server});



	set_table_index(&inspircd4_protocol_commands, STRING("PING"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_ping});
	set_table_index(&inspircd4_protocol_commands, STRING("PONG"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_pong});

	set_table_index(&inspircd4_protocol_commands, STRING("SERVER"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_server});
	set_table_index(&inspircd4_protocol_commands, STRING("SQUIT"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_squit});
	set_table_index(&inspircd4_protocol_commands, STRING("RSQUIT"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_rsquit});

	set_table_index(&inspircd4_protocol_commands, STRING("UID"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_uid});
	set_table_index(&inspircd4_protocol_commands, STRING("NICK"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_nick});
	set_table_index(&inspircd4_protocol_commands, STRING("QUIT"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_quit});
	set_table_index(&inspircd4_protocol_commands, STRING("KILL"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_kill});
	set_table_index(&inspircd4_protocol_commands, STRING("OPERTYPE"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_opertype});

	set_table_index(&inspircd4_protocol_commands, STRING("FJOIN"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_fjoin});
	set_table_index(&inspircd4_protocol_commands, STRING("IJOIN"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_ijoin});
	set_table_index(&inspircd4_protocol_commands, STRING("PART"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_part});
	set_table_index(&inspircd4_protocol_commands, STRING("KICK"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_kick});

	set_table_index(&inspircd4_protocol_commands, STRING("SQUERY"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_squery});
	set_table_index(&inspircd4_protocol_commands, STRING("PRIVMSG"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_privmsg});
	set_table_index(&inspircd4_protocol_commands, STRING("NOTICE"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_notice});

	set_table_index(&inspircd4_protocol_commands, STRING("MODE"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_mode});
	set_table_index(&inspircd4_protocol_commands, STRING("FMODE"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_fmode});

	set_table_index(&inspircd4_protocol_commands, STRING("METADATA"), (union table_ptr){.function = (void (*)(void))&inspircd4_protocol_handle_metadata});

	return 0;
}

void init_inspircd4_protocol_fail(void) {
	clear_table(&inspircd4_protocol_commands);
	free(inspircd4_protocol_commands.array);
}
...
void * inspircd4_protocol_connection(void *type) {
	...
	if (!is_incoming) {
		networks[net].send(handle, STRING("CAPAB START 1206\nCAPAB END\n"));
	...
	while (1) {
		...
		size_t old_len;
		{
			...
			while (1) {
				...
				if (err >= 2) { // Connection closed, or some uncorrected error
					goto inspircd4_protocol_handle_connection_close;
		...
		while (1) {
			...
			if (ready) {
				WRITES(2, STRING("[InspIRCd v4] ["));
				WRITES(2, config->name);
				WRITES(2, STRING(" -> us] Got `"));
			} else {
				WRITES(2, STRING("[InspIRCd v4] [unidentified server -> us] Got `"));
			}
			WRITES(2, line);
			WRITES(2, STRING("'\r\n"));
			...
			if (msg_len == offset) {
				WRITES(2, STRING("[InspIRCd v4] Protocol violation: empty message.\r\n\n"));
				goto inspircd4_protocol_handle_connection_close;
			}
			...
			if (full_msg.data[offset] == ':') {
				...
				if (source.len == 0) {
					WRITES(2, STRING("[InspIRCd v4] Protocol violation: source prefix but no source.\r\n\n"));
					goto inspircd4_protocol_handle_connection_close;
				}
				if (!found || offset >= msg_len) {
					WRITES(2, STRING("[InspIRCd v4] Protocol violation: source but no command.\r\n\n"));
					goto inspircd4_protocol_handle_connection_close;
				}

				if (STRING_EQ(source, SID)) {
					WRITES(2, STRING("[InspIRCd v4] Protocol violation: other server sent us as source!\r\n\n"));
					goto inspircd4_protocol_handle_connection_close;
				}
			} else {
				if (ready)
			...
			if (!ready) {
				...
				char exists;
				func = (int (*)(struct string, size_t, struct string *, size_t, void *, struct server_config **, char))get_table_index(inspircd4_protocol_init_commands, command, &exists).function;
				if (!func) {
				...
				int (*func)(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming);
				char exists;
				func = (int (*)(struct string, size_t, struct string *, size_t, void *, struct server_config *, char))get_table_index(inspircd4_protocol_commands, command, &exists).function;
				if (!exists) {
					WRITES(2, STRING("WARNING: Command is unknown, ignoring.\r\n"));
					goto inspircd4_protocol_handle_connection_unlock_next;
				}
			...
			inspircd4_protocol_handle_connection_unlock_next:
			WRITES(2, STRING("\n"));
	...
	inspircd4_protocol_handle_connection_unlock_close:
	mutex_unlock(&state_lock);
	inspircd4_protocol_handle_connection_close:
	free(full_msg.data);

	if (ready) {
		mutex_lock(&(state_lock));
		char exists;
		unlink_server(config->sid, get_table_index(server_list, config->sid, &exists).data, self, INSPIRCD4_PROTOCOL);
		mutex_unlock(&(state_lock));
	}
...
void * inspircd4_protocol_autoconnect(void *tmp) {
	...
	type->protocol = INSPIRCD4_PROTOCOL;
	type->is_incoming = 0;
	info->type = type;
	info->config = config;
	...
	while (1) {
		...
		inspircd4_protocol_connection(info);
	}
}
...
void inspircd4_protocol_update_propagations_inner(struct server_info *source) {
	for (size_t i = 0; i < source->connected_to.len; i++) {
		...
		if (adjacent->distance == 0 && !STRING_EQ(adjacent->sid, SID)) {
			...
			inspircd4_protocol_update_propagations_inner(adjacent);
		}
	}
}
...
void inspircd4_protocol_update_propagations(void) {
	for (size_t i = 0; i < server_list.len; i++) {
		...
		if (other->protocol == INSPIRCD4_PROTOCOL) {
			other->distance = 0;
		}
	}

	inspircd4_protocol_update_propagations_inner(self);
}
...
void inspircd4_protocol_propagate(struct string from, struct string msg) {
	for (size_t i = 0; i < self->connected_to.len; i++) {
		...
		if (adjacent->protocol != INSPIRCD4_PROTOCOL || STRING_EQ(from, adjacent->sid))
...
void inspircd4_protocol_propagate_new_server(struct string from, struct string attached_to, struct server_info *info) {
	...
	if (info->protocol == INSPIRCD4_PROTOCOL)
		inspircd4_protocol_propagate(from, attached_to);
	...
	inspircd4_protocol_propagate(from, STRING(" SERVER "));
	inspircd4_protocol_propagate(from, info->name);
	inspircd4_protocol_propagate(from, STRING(" "));
	inspircd4_protocol_propagate(from, info->sid);
	inspircd4_protocol_propagate(from, STRING(" :"));
	inspircd4_protocol_propagate(from, info->fullname);
	inspircd4_protocol_propagate(from, STRING("\n"));

	inspircd4_protocol_propagate(from, STRING(":"));
	inspircd4_protocol_propagate(from, info->sid);
	inspircd4_protocol_propagate(from, STRING(" BURST "));
	...
	if (current < 0 || err) {
		inspircd4_protocol_propagate(from, STRING("1"));
	} else {
		inspircd4_protocol_propagate(from, current_time);
		free(current_time.data);
	}

	inspircd4_protocol_propagate(from, STRING("\n:"));
	inspircd4_protocol_propagate(from, info->sid);
	inspircd4_protocol_propagate(from, STRING(" ENDBURST\n"));
}
...
void inspircd4_protocol_propagate_new_user(struct string from, struct user_info *info) {
	...
	inspircd4_protocol_propagate(from, STRING(" UID "));
	inspircd4_protocol_propagate(from, info->uid);
	inspircd4_protocol_propagate(from, STRING(" "));
	inspircd4_protocol_propagate(from, info->nick_ts_str);
	inspircd4_protocol_propagate(from, STRING(" "));
	inspircd4_protocol_propagate(from, info->nick);
	inspircd4_protocol_propagate(from, STRING(" "));
	...
	inspircd4_protocol_propagate(from, STRING(" "));
	inspircd4_protocol_propagate(from, info->address);
	inspircd4_protocol_propagate(from, STRING(" "));
	inspircd4_protocol_propagate(from, info->user_ts_str);
	inspircd4_protocol_propagate(from, STRING(" + :"));
	inspircd4_protocol_propagate(from, info->fullname);
	inspircd4_protocol_propagate(from, STRING("\n"));
}
...
void inspircd4_protocol_propagate_rename_user(struct string from, struct user_info *user, struct string nick, size_t timestamp, struct string timestamp_str, char forced, char immediate) {
	if (forced) {
		if (STRING_EQ(user->uid, nick)) {
			inspircd4_protocol_propagate(from, STRING(":"));
			inspircd4_protocol_propagate(from, from);
			inspircd4_protocol_propagate(from, STRING(" SAVE "));
			inspircd4_protocol_propagate(from, user->uid);
			inspircd4_protocol_propagate(from, STRING(" :"));
			inspircd4_protocol_propagate(from, user->nick_ts_str);
			inspircd4_protocol_propagate(from, STRING("\n"));
		} else {
			inspircd4_protocol_propagate(from, STRING(":"));
			inspircd4_protocol_propagate(from, from);
			inspircd4_protocol_propagate(from, STRING(" SANICK "));
			inspircd4_protocol_propagate(from, user->uid);
			inspircd4_protocol_propagate(from, STRING(" :"));
			inspircd4_protocol_propagate(from, nick);
			inspircd4_protocol_propagate(from, STRING("\n"));
		}
	} else {
		inspircd4_protocol_propagate(from, STRING(":"));
		inspircd4_protocol_propagate(from, user->uid);
		inspircd4_protocol_propagate(from, STRING(" NICK "));
		inspircd4_protocol_propagate(from, nick);
		inspircd4_protocol_propagate(from, STRING(" "));
		inspircd4_protocol_propagate(from, timestamp_str);
		inspircd4_protocol_propagate(from, STRING("\n"));
	}
}
...
void inspircd4_protocol_propagate_remove_user(struct string from, struct user_info *info, struct string reason) {
	inspircd4_protocol_propagate(from, STRING(":"));
	inspircd4_protocol_propagate(from, info->uid);
	inspircd4_protocol_propagate(from, STRING(" QUIT :"));
	inspircd4_protocol_propagate(from, reason);
	inspircd4_protocol_propagate(from, STRING("\n"));
}
...
void inspircd4_protocol_propagate_kill_user(struct string from, struct string source, struct user_info *info, struct string reason) {
	inspircd4_protocol_propagate(from, STRING(":"));
	inspircd4_protocol_propagate(from, source);
	inspircd4_protocol_propagate(from, STRING(" KILL "));
	inspircd4_protocol_propagate(from, info->uid);
	inspircd4_protocol_propagate(from, STRING(" :"));
	inspircd4_protocol_propagate(from, reason);
	inspircd4_protocol_propagate(from, STRING("\n"));
...
void inspircd4_protocol_propagate_oper_user(struct string from, struct user_info *user, struct string type, struct string source) {
	if (type.len == 0) {
		inspircd4_protocol_propagate(from, STRING(":"));
		inspircd4_protocol_propagate(from, source);
		inspircd4_protocol_propagate(from, STRING(" MODE "));
		inspircd4_protocol_propagate(from, user->uid);
		inspircd4_protocol_propagate(from, STRING(" -o\n"));
	} else {
		inspircd4_protocol_propagate(from, STRING(":"));
		inspircd4_protocol_propagate(from, user->uid);
		inspircd4_protocol_propagate(from, STRING(" OPERTYPE :"));
		inspircd4_protocol_propagate(from, type);
		inspircd4_protocol_propagate(from, STRING("\n"));
	}
}
...
void inspircd4_protocol_propagate_set_account(struct string from, struct user_info *user, struct string account, struct string source) {
	inspircd4_protocol_propagate(from, STRING(":"));
	inspircd4_protocol_propagate(from, source);
	inspircd4_protocol_propagate(from, STRING(" METADATA "));
	inspircd4_protocol_propagate(from, user->uid);
	inspircd4_protocol_propagate(from, STRING(" accountname :"));
	inspircd4_protocol_propagate(from, account);
	inspircd4_protocol_propagate(from, STRING("\n"));
}
...
void inspircd4_protocol_propagate_set_cert(struct string from, struct user_info *user, struct string cert, struct string source) {
	...
	inspircd4_protocol_propagate(from, user->uid);
	if (cert.len != 0) {
		inspircd4_protocol_propagate(from, STRING(" ssl_cert :vTrse "));
		inspircd4_protocol_propagate(from, cert);
		inspircd4_protocol_propagate(from, STRING("\n"));
	} else {
		inspircd4_protocol_propagate(from, STRING(" ssl_cert :vtrsE No certificate was found.\n"));
	}
}

// [:source] FJOIN <channel> <timestamp> <modes> [<mode args>] <userlist: modes,uid:mid [...]>
void inspircd4_protocol_propagate_set_channel(struct string from, struct channel_info *channel, char is_new_server, size_t user_count, struct user_info **users) {
	inspircd4_protocol_propagate(from, STRING(":"));
	inspircd4_protocol_propagate(from, SID);
	inspircd4_protocol_propagate(from, STRING(" FJOIN "));
	inspircd4_protocol_propagate(from, channel->name);
	inspircd4_protocol_propagate(from, STRING(" "));
	inspircd4_protocol_propagate(from, channel->channel_ts_str);
	inspircd4_protocol_propagate(from, STRING(" + :"));
	for (size_t x = 0; x < user_count; x++) {
		inspircd4_protocol_propagate(from, STRING(","));
		inspircd4_protocol_propagate(from, users[x]->uid);
		...
		if (member)
			...
			inspircd4_protocol_propagate(from, STRING("0"));
...
void inspircd4_protocol_propagate_part_channel(struct string from, struct channel_info *channel, struct user_info *user, struct string reason) {
	inspircd4_protocol_propagate(from, STRING(":"));
	inspircd4_protocol_propagate(from, user->uid);
	inspircd4_protocol_propagate(from, STRING(" PART "));
	inspircd4_protocol_propagate(from, channel->name);
	inspircd4_protocol_propagate(from, STRING(" :"));
	inspircd4_protocol_propagate(from, reason);
	inspircd4_protocol_propagate(from, STRING("\n"));
...
void inspircd4_protocol_propagate_kick_channel(struct string from, struct string source, struct channel_info *channel, struct user_info *user, struct string reason) {
	inspircd4_protocol_propagate(from, STRING(":"));
	inspircd4_protocol_propagate(from, source);
	inspircd4_protocol_propagate(from, STRING(" KICK "));
	inspircd4_protocol_propagate(from, channel->name);
	inspircd4_protocol_propagate(from, STRING(" "));
	inspircd4_protocol_propagate(from, user->uid);
	inspircd4_protocol_propagate(from, STRING(" :"));
...
void inspircd4_protocol_propagate_privmsg(struct string from, struct string source, struct string target, struct string msg) {
	...
	if (user || server) {
		...
		if (target_server->protocol != INSPIRCD4_PROTOCOL || STRING_EQ(target_server->sid, SID))
			return;
		...
		inspircd4_protocol_propagate(from, STRING(":"));
		inspircd4_protocol_propagate(from, source);
		inspircd4_protocol_propagate(from, STRING(" PRIVMSG "));
		inspircd4_protocol_propagate(from, target);
		inspircd4_protocol_propagate(from, STRING(" :"));
		inspircd4_protocol_propagate(from, msg);
		inspircd4_protocol_propagate(from, STRING("\n"));
	}
}
...
void inspircd4_protocol_propagate_notice(struct string from, struct string source, struct string target, struct string msg) {
	...
	if (user || server) {
		...
		inspircd4_protocol_propagate(from, STRING(":"));
		inspircd4_protocol_propagate(from, source);
		inspircd4_protocol_propagate(from, STRING(" NOTICE "));
		inspircd4_protocol_propagate(from, target);
		inspircd4_protocol_propagate(from, STRING(" :"));
		inspircd4_protocol_propagate(from, msg);
		inspircd4_protocol_propagate(from, STRING("\n"));
...
int inspircd4_protocol_handle_new_user(struct string from, struct user_info *info) {
	...
	info->protocol_specific[INSPIRCD4_PROTOCOL] = prot_info;
...
void inspircd4_protocol_handle_remove_user(struct string from, struct user_info *info, struct string reason, char propagate) {
	...
	while (prot_info->memberships.len > 0) {
		char exists;
		struct inspircd4_protocol_member_id *mid = get_and_remove_table_index(&(prot_info->memberships), prot_info->memberships.array[0].name, &exists).data;
...
void inspircd4_protocol_fail_new_user(struct string from, struct user_info *info) {
	...
	for (size_t i = 0; i < prot_info->memberships.len; i++) {
		struct inspircd4_protocol_member_id *member = prot_info->memberships.array[i].ptr.data;
...
void inspircd4_protocol_do_unlink_inner(struct string from, struct server_info *target, struct string reason) {
	...
	while (target->connected_to.len > i) {
		...
		inspircd4_protocol_do_unlink_inner(from, adjacent, reason);
...
void inspircd4_protocol_do_unlink(struct string from, struct server_info *a, struct server_info *b) {
	...
	if (a->distance == 0 && !STRING_EQ(a->sid, SID)) {
		inspircd4_protocol_do_unlink_inner(from, a, reason);
		remove_server(from, a, reason);
	} else {
		inspircd4_protocol_do_unlink_inner(from, b, reason);
...
void inspircd4_protocol_introduce_servers_to_inner(size_t net, void *handle, struct string source, struct server_info *target) {
	networks[net].send(handle, STRING(":"));
	...
	for (size_t i = 0; i < target->connected_to.len; i++) {
		...
		if (adjacent->distance > target->distance) {
			inspircd4_protocol_introduce_servers_to_inner(net, handle, target->sid, adjacent);
		}
	}
}

void inspircd4_protocol_introduce_servers_to(size_t net, void *handle) {
	for (size_t i = 0; i < self->connected_to.len; i++) {
		struct server_info *info = self->connected_to.array[i].ptr.data;
		if (info->protocol == INSPIRCD4_PROTOCOL) { // This server hasn't been added to the list yet, so no need to check for that
			inspircd4_protocol_introduce_servers_to_inner(net, handle, SID, info);
		}
	}
...
void inspircd4_protocol_introduce_user_to(size_t net, void *handle, struct user_info *user, char join_channels) {
	networks[net].send(handle, STRING(":"));
...
void inspircd4_protocol_introduce_channel_to(size_t net, void *handle, struct channel_info *channel) {
	networks[net].send(handle, STRING(":"));
	networks[net].send(handle, SID);
	networks[net].send(handle, STRING(" FJOIN "));
	networks[net].send(handle, channel->name);
...
int inspircd4_protocol_init_handle_capab(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config **config, char is_incoming) {
	if (argc < 1) {
		WRITES(2, STRING("[InspIRCd v4] Invalid CAPAB received! (Missing parameters)\r\n"));
		return -1;
	}
...
int inspircd4_protocol_init_handle_server(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config **config, char is_incoming) {
	...
	if (source.len != 0) {
		WRITES(2, STRING("[InspIRCd v4] Server attempting to use a source without having introduced itself!\r\n"));
		return -1;
	}

	if (is_incoming) {
		char exists;
		*config = get_table_index(server_config, argv[2], &exists).data;
		if (!exists) {
	...
	if (!STRING_EQ(argv[1], (*config)->in_pass)) {
		WRITES(2, STRING("[InspIRCd v4] WARNING: Server supplied the wrong password!\r\n"));
		return -1;
	}
	...
	if (res != 0) {
		WRITES(2, STRING("[InspIRCd v4] ERROR: OOM, severing link.\r\n"));
		return -1;
	}
	...
	inspircd4_protocol_introduce_servers_to(net, handle);

	for (size_t i = 0; i < user_list.len; i++)
		inspircd4_protocol_introduce_user_to(net, handle, user_list.array[i].ptr.data, 0);

	for (size_t i = 0; i < channel_list.len; i++)
		inspircd4_protocol_introduce_channel_to(net, handle, channel_list.array[i].ptr.data);
...
int inspircd4_protocol_handle_ping(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	if (argc < 1) {
		WRITES(2, STRING("[InspIRCd v4] Invalid PING received! (Missing parameters)\r\n"));
		return -1;
	}
...
int inspircd4_protocol_handle_server(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	...
	if (has_table_index(server_list, argv[1])) {
		WRITES(2, STRING("[InspIRCd v4] Duplicate SERVER attempted to be created!\r\n"));
		return -1;
	}

	if (add_server(config->sid, source, argv[1], argv[0], argv[argc - 1], INSPIRCD4_PROTOCOL, net, handle) != 0) {
		WRITES(2, STRING("ERROR: Unable to add server!\r\n"));
		return -1;
	}
...
int inspircd4_protocol_handle_squit(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	if (argc < 1) {
		WRITES(2, STRING("[InspIRCd v4] Invalid SQUIT received! (Missing parameters)\r\n"));
		return -1;
	}
	...
	if (!a || !b) { // Maybe we already RSQUIT it or smth
		WRITES(2, STRING("[InspIRCd v4] Invalid SQUIT received! (Unknown source or target)\r\n"));
		return -1;
	}
	if (a->protocol != INSPIRCD4_PROTOCOL || b->protocol != INSPIRCD4_PROTOCOL) { // They're trying to use SQUIT for some unrelated server...
		WRITES(2, STRING("[InspIRCd v4] Invalid SQUIT received! (Bad SID or source)\r\n"));
		return -1;
	}

	unlink_server(config->sid, a, b, INSPIRCD4_PROTOCOL);
...
int inspircd4_protocol_handle_rsquit(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	if (argc < 1) {
		WRITES(2, STRING("[InspIRCd v4] Invalid RSQUIT received! (Missing parameters)\r\n"));
		return -1;
	}
...
int inspircd4_protocol_handle_uid(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	...
	if (has_table_index(user_list, argv[0])) {
		WRITES(2, STRING("[InspIRCd v4] Duplicate UID attempted to be created!\r\n"));
		return -1;
	}
	...
	while (1) {
		...
		switch(argv[9].data[mode_i]) {
			...
			default:
				if (dir == '?') {
					WRITES(2, STRING("[InspIRCd v4] Invalid UID received! (Mode direction not set)\r\n"));
					return -1;
				}
				...
				switch(inspircd4_protocol_user_mode_types[(unsigned char)argv[9].data[mode_i]]) {
					...
					case MODE_TYPE_MULTIPLE:
						...
						break;
					default:
						WRITES(2, STRING("[InspIRCd v4] Invalid UID received! (Unknown mode given)\r\n"));
						return -1;
				}
		}
	...
	if (arg_i >= argc) {
		WRITES(2, STRING("[InspIRCd v4] Invalid UID received! (Missing mode arguments)\r\n"));
		return -1;
	}
	...
	if (err) {
		WRITES(2, STRING("[InspIRCd v4] Invalid UID received! (Invalid nick timestamp)\r\n"));
		return -1;
	}
...
int inspircd4_protocol_handle_nick(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	if (argc < 2) {
		WRITES(2, STRING("[InspIRCd v4] Invalid NICK received! (Missing parameters)\r\n"));
		return -1;
	}
	...
	size_t nick_ts = str_to_unsigned(argv[1], &err);
	if (err) {
		WRITES(2, STRING("[InspIRCd v4] Invalid NICK received! (Invalid timestamp)\r\n"));
		return -1;
	}
...
int inspircd4_protocol_handle_quit(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	...
	if (STRING_EQ(user->server, SID)) {
		WRITES(2, STRING("[InspIRCd v4] Invalid QUIT received! (Attempting to quit a local user)\r\n"));
		return -1;
	}
...
int inspircd4_protocol_handle_kill(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	if (argc < 1) {
		WRITES(2, STRING("[InspIRCd v4] Invalid KILL received! (Missing parameters)\r\n"));
		return -1;
	}
	...
	if (ignore)
		inspircd4_protocol_introduce_user_to(net, handle, user, 1);
...
int inspircd4_protocol_handle_opertype(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	if (argc < 1) {
		WRITES(2, STRING("[InspIRCd v4] Invalid OPERTYPE received! (Missing parameters)\r\n"));
		return -1;
	}
	...
	if (oper_user(config->sid, user, argv[0], config->sid) != 0) {
		WRITES(2, STRING("[InspIRCd v4] ERROR: Unable to set oper type!\r\n"));
		return -1;
	}
...
int inspircd4_protocol_handle_fjoin(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	if (argc < 4) {
		WRITES(2, STRING("[InspIRCd v4] Invalid FJOIN received! (Missing parameters)\r\n"));
		return -1;
	}
	...
	if (err) {
		WRITES(2, STRING("[InspIRCd v4] Invalid FJOIN received! (Invalid timestamp)\r\n"));
		return -1;
	}
	...
	for (size_t i = 0; i < argv[2].len; i++) {
		switch(argv[2].data[i]) {
			...
			default:
				if (dir == '?') {
					WRITES(2, STRING("[InspIRCd v4] Invalid FJOIN received (Mode direction not set)\r\n"));
					return -1;
				}
				switch(inspircd4_protocol_channel_mode_types[(unsigned char)argv[2].data[i]]) {
					case MODE_TYPE_NOARGS:
						break;
					case MODE_TYPE_REPLACE:
					case MODE_TYPE_MODE:
					...
					case MODE_TYPE_USERS:
						WRITES(2, STRING("[InspIRCd v4] Invalid FJOIN received! (User mode put in the modes instead of the user list)\r\n"));
						return -1;
					default:
						WRITES(2, STRING("[InspIRCd v4] Invalid FJOIN received! (Unknown mode given)\r\n"));
						return -1;
				}
		}
	}
	...
	if (!users && user_count != 0) {
		WRITES(2, STRING("[InspIRCd v4] [FJOIN] OOM! Disconnecting server.\r\n"));
		return -1;
	}

	struct inspircd4_protocol_member_id **members;
	members = malloc(sizeof(*members) * user_count * 2);
	if (!members && user_count != 0)
		goto inspircd4_protocol_handle_fjoin_free_users;
	...
	for (size_t i = 0; i < argv[arg_i].len; n++) {
		...
		if (!exists || !users[n]->protocol_specific[INSPIRCD4_PROTOCOL]) // TODO: Check that it's coming the right way too
			user_count--;
		...
		if (!exists || !users[n]->protocol_specific[INSPIRCD4_PROTOCOL]) {
			n--;
			continue;
		}
	...
	for (n = 0; n < user_count; n++) {
		struct inspircd4_protocol_specific_user *this = users[n]->protocol_specific[INSPIRCD4_PROTOCOL];
		char exists;
		...
		if (set_table_index(&(this->memberships), argv[0], (union table_ptr){.data = members[n]}) != 0)
			goto inspircd4_protocol_handle_fjoin_reset_member_ids;
	}
	...
	n = user_count;
	inspircd4_protocol_handle_fjoin_free_member_ids:
	for (size_t x = n; n > 0;) {
		n--;
	...
	free(members);
	inspircd4_protocol_handle_fjoin_free_users:
	free(users);
	return -1;
}
...
int inspircd4_protocol_handle_ijoin(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	if (argc < 2) {
		WRITES(2, STRING("[InspIRCd v4] Invalid IJOIN received! (Missing parameters)\r\n"));
		return -1;
	}
	...
	struct inspircd4_protocol_specific_user *this = user->protocol_specific[INSPIRCD4_PROTOCOL];
...
int inspircd4_protocol_handle_part(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	if (argc < 1) {
		WRITES(2, STRING("[InspIRCd v4] Invalid PART received! (Missing parameters)\r\n"));
		return -1;
	}
	struct string reason;
	if (argc < 2)
...
int inspircd4_protocol_handle_kick(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	...
	if (argc >= 4) {
		if (uses_inspircd4) {
			...
			struct inspircd4_protocol_specific_user *prot_specific = user->protocol_specific[INSPIRCD4_PROTOCOL];
			char exists;
			struct inspircd4_protocol_member_id *current_member_id = get_table_index(prot_specific->memberships, channel->name, &exists).data;
...
int inspircd4_protocol_handle_squery(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	if (argc < 2) {
		WRITES(2, STRING("[InspIRCd v4] Invalid SQUERY received! (Missing parameters)\r\n"));
		return -1;
	}

	privmsg(config->sid, source, argv[0], argv[1]); // BUG: SQUERY shouldn't be *literally* handled like PRIVMSG because it's unicast-only
...
int inspircd4_protocol_handle_privmsg(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	if (argc < 2) {
		WRITES(2, STRING("[InspIRCd v4] Invalid PRIVMSG received! (Missing parameters)\r\n"));
		return -1;
	}
...
int inspircd4_protocol_handle_notice(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	if (argc < 2) {
		WRITES(2, STRING("[InspIRCd v4] Invalid NOTICE received! (Missing parameters)\r\n"));
		return -1;
	}
...
int inspircd4_protocol_handle_mode(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	if (argc < 2) {
		WRITES(2, STRING("[InspIRCd v4] Invalid MODE received! (Missing parameters)\r\n"));
		return -1;
	}
	...
	if (user) {
		...
		for (size_t i = 0; i < argv[1].len; i++) {
			switch(argv[1].data[i]) {
				...
				default:
					if (dir == '?') {
						WRITES(2, STRING("[InspIRCd v4] Invalid MODE received (Mode direction not set)\r\n"));
						return -1;
					}
					switch(inspircd4_protocol_user_mode_types[(unsigned char)argv[1].data[i]]) {
						case MODE_TYPE_NOARGS:
						...
						case MODE_TYPE_USERS:
							...
							break;
						default:
							WRITES(2, STRING("[InspIRCd v4] Invalid MODE received! (Unknown mode given)\r\n"));
							return -1;
					}
			}
		}
	...
	for (size_t i = 0; i < argv[2].len; i++) {
		switch(argv[2].data[i]) {
			...
			default:
				...
				switch(inspircd4_protocol_user_mode_types[(unsigned char)argv[2].data[i]]) {
					case MODE_TYPE_NOARGS:
...
int inspircd4_protocol_handle_metadata(struct string source, size_t argc, struct string *argv, size_t net, void *handle, struct server_config *config, char is_incoming) {
	if (argc < 3) {
		WRITES(2, STRING("[InspIRCd v4] Invalid METADATA received! (Missing parameters)\r\n"));
		return -1;
	}
